...
mult_runs = function(N, cohorting = T, visitors = T, temp_staff = T, rel_trans_common = 1/4, rel_trans_staff = 1/4, 
                     ...
                     p_subclin_nonres = 0, p_subclin_res = 0, daily_attack_unvax = 0.18, daily_attack_vax = 0.18, staff_vax_req = F, 
                     ...
                     n_contact_staff = 5, n_start = 1, days_inf = 5, mult_asymp_res = 1, mult_asymp_nonres = 1, seed_asymp = F, 
                     isolate = T, time = 30, test = T, test_sens = 0.7, test_frac = 0.9, test_days = '2x_week', 
                     test_type = 'all', test_start_day = 1, visit_test = T, start_mult = 1, staff_prob = 0.0015, visit_prob = 0.0015,
...
s.n_tot = 600
s.daily_attack_unvax = .18; s.daily_attack_vax = .18
...
s.n_contact_common_res = 3; s.n_contact_common_staff = 3; s.n_contact_staff = 5
s.p_asymp_nonres = 0; s.p_asymp_res = 0
s.p_subclin_nonres = 0; s.p_subclin_res = 0
s.test_sens = 1; s.test_frac = 1; s.test_days = "2x_week"; s.test_type = "all"; s.test_start_day = 1
...
make_df = function(disperse = T, # how to distribute runs
  ...
  # make a grid
  df = expand_grid(daily_attack_unvax, daily_attack_vax, n_tot, disperse_transmission, start_mult, n_start, mult_asymp_res,
                   ...
                   quarantine.length, quarantine, seed_asymp, overdisp_off) 
  ...
  df = df %>%
    ...
    filter((test_days=="2x_week" & test_sens == 1 & test_frac == 1 & test_type == "all") | test)
...
sims = function(df, i, sched, cohorts){
  ...
  out = mult_runs(df = sched, cohorts = cohorts, N = 1, daily_attack_unvax = df$daily_attack_unvax[i], daily_attack_vax = df$daily_attack_vax[i], 
                  ...
                  seed_asymp = df$seed_asymp[i], overdisp_off = df$overdisp_off[i]) %>%
