...
output <- output[,.(inf_ct_sympR_R_room.sum = sum(inf_ct_sympR_R_room, na.rm = TRUE), inf_ct_asympR_R_room.sum = sum(inf_ct_asympR_R_room, na.rm = TRUE), inf_ct_sympR_S_room.sum = sum(inf_ct_sympR_S_room, na.rm = TRUE), inf_ct_asympR_S_room.sum = sum(inf_ct_asympR_S_room, na.rm = TRUE), inf_ct_S_S_room.sum = sum(inf_ct_S_S_room, na.rm = TRUE), inf_ct_sympS_R_room.sum = sum(inf_ct_sympS_R_room, na.rm = TRUE), inf_ct_asympS_R_room.sum = sum(inf_ct_asympS_R_room, na.rm = TRUE),
                    ...
                    inf_ct_sympR_V_common.sum = sum(inf_ct_sympR_V_common, na.rm = TRUE), inf_ct_asympR_V_common.sum = sum(inf_ct_asympR_V_common, na.rm = TRUE), inf_ct_sympS_V_common.sum = sum(inf_ct_sympS_V_common, na.rm = TRUE), inf_ct_asympS_V_common.sum = sum(inf_ct_asympS_V_common, na.rm = TRUE), inf_ct_sympV_R_common.sum = sum(inf_ct_sympV_R_common, na.rm = TRUE), inf_ct_asympV_R_common.sum = sum(inf_ct_asympV_R_common, na.rm = TRUE), inf_ct_sympV_S_common.sum = sum(inf_ct_sympV_S_common, na.rm = TRUE), inf_ct_asympV_S_common.sum = sum(inf_ct_asympV_S_common, na.rm = TRUE),
                    ...
                    risk_ct_sympR_V_common.sum = sum(risk_ct_sympR_V_common, na.rm = TRUE), risk_ct_asympR_V_common.sum = sum(risk_ct_asympR_V_common, na.rm = TRUE), risk_ct_sympS_V_common.sum = sum(risk_ct_sympS_V_common, na.rm = TRUE), risk_ct_asympS_V_common.sum = sum(risk_ct_asympS_V_common, na.rm = TRUE), risk_ct_sympV_V_common.sum = sum(risk_ct_sympV_V_common, na.rm = TRUE), risk_ct_asympV_V_common.sum = sum(risk_ct_asympV_V_common, na.rm = TRUE), risk_ct_sympV_R_common.sum = sum(risk_ct_sympV_R_common, na.rm = TRUE), risk_ct_asympV_R_common.sum = sum(risk_ct_asympV_R_common, na.rm = TRUE), risk_ct_sympV_S_common.sum = sum(risk_ct_sympV_S_common, na.rm = TRUE), risk_ct_asympV_S_common.sum = sum(risk_ct_asympV_S_common, na.rm = TRUE),
...
sar.compare <- expand.grid(location = c("Room", "Common area", "Staff interactions"), source = c("res", "staff", "visit"), susp = c("res", "staff", "visit"), comorbid = c(0,1,2), symptomatic = c(T,F), isolate = c(T,F)) %>% left_join(output, by = "group") %>%
  mutate(expected.sar = attack*
           ...
           ifelse(location == "Common area", rel_trans_common*ifelse(source=="res" & symptomatic & !isolate, rel_trans_room_symp_res, 1), 1)*
  ...
  mutate(observed.sar = NA, 
         observed.sar = ifelse(location == "Room",
                ifelse(source=="res", 
                       ifelse(susp=="res", 
                              ifelse(symptomatic, 
                ...
                ifelse(source=="visit",
                       ...
                       observed.sar),
                observed.sar),
         observed.sar = ifelse(location == "Common area",
                               ifelse(source=="res", 
                                      ifelse(susp=="res", 
                                             ifelse(symptomatic, 
                                      ...
                                      ifelse(susp=="visit",
                                             ifelse(symptomatic,
                                                    inf_ct_sympR_V_common.sum/risk_ct_sympR_V_common.sum,
                                                    inf_ct_asympR_V_common.sum/risk_ct_asympR_V_common.sum),
                                             observed.sar),
                                      observed.sar),
                               ifelse(source=="staff",
                                      ifelse(susp=="res", 
                                      ...
                                      ifelse(susp=="visit",
                                             ifelse(symptomatic,
                                                    inf_ct_sympS_V_common.sum/risk_ct_sympS_V_common.sum,
                                                    inf_ct_asympS_V_common.sum/risk_ct_asympS_V_common.sum),
                                             observed.sar),
                                      observed.sar),
                               ifelse(source=="visit",
                                      ifelse(susp=="res",
                                             ifelse(symptomatic,
                                                    inf_ct_sympV_R_common.sum/risk_ct_sympV_R_common.sum,
                                                    inf_ct_asympV_R_common.sum/risk_ct_asympV_R_common.sum),
                                             observed.sar),
                                      ifelse(susp=="staff",
                                             ifelse(symptomatic,
                                                    inf_ct_sympV_S_common.sum/risk_ct_sympV_S_common.sum,
                                                    inf_ct_asympV_S_common.sum/risk_ct_asympV_S_common.sum),
                                             observed.sar),
                                      ifelse(susp=="visit",
                                             ifelse(symptomatic,
                                                    inf_ct_sympV_V_common.sum/risk_ct_sympV_V_common.sum,
                                                    inf_ct_asympV_V_common.sum/risk_ct_asympV_V_common.sum),
                                             observed.sar),
                                      observed.sar),
                               observed.sar),
         observed.sar = ifelse(location == "Staff interactions",
                               ifelse(source=="staff",
                                      ifelse(susp=="staff",
                                             ifelse(symptomatic,
